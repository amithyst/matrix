...
已授权的回退会把坏点移出活动序列并写入最多 64 条 tombstone 审计记录，再冷重启到
上一点；没有更早点时回 home，无 home 才回地图默认点。同一次顶层 exec 链最多自动回退
一次，后续 fall/teleport/F9 重启也不会清零该门禁；上一点仍失败就停机并保留其存档，
避免全局 SONIC/代码故障连续清空历史。
