...
| 按住 **Ctrl** 或 **Alt** + WASD | 原生 mode 1 `SLOW_WALK`，基础目标 0.20 m/s |
| 不带速度修饰键的 WASD | 原生 mode 2 `WALK`，目标 0.80 m/s |
| 按住 **Shift** + WASD | 原生 mode 3 `RUN`，目标 2.50 m/s |
| 连按两次同一个 **W/A/S/D** | 当前 slow/walk/run 档提升到 0.30/1.00/2.75 m/s，松键退出 boost |
...
| **Q / E** | 不按 WASD 时让机器人原地向左 / 向右转向；不会产生平移 |
| **F6** | 在输入完全回中时循环切换“相机朝向 / 相机侧移 / 机身相对” |

左右 Ctrl、Alt、Shift 分别等价；Ctrl 和 Alt 都选择 slow。slow 修饰键与 Shift 同时按下时
优先使用更慢的静走档；切换 slow/walk/run 档会清除双击候选和已激活 boost。

默认“相机朝向”沿用原行为：WASD 相对相机，机器人先朝运动方向转身，大角度转身
尚未完成时会主动降低平移速度。“相机侧移”仍按相机坐标解释 WASD，但保持机器人身体
朝向；“机身相对”则完全忽略相机，以机器人当前身体坐标解释 WASD，并保持身体朝向。
后两种模式会向 SONIC 分别发送移动方向和身体朝向，首次实机使用应从 slow 档开始验收。
...
调节，默认标称值 120；三种移动模式也在该页面直接选择。所有值和移动模式都会写入
主机独立的 `motion-control.json`，跨重启和星体传送保留。ESC 面板打开期间箭头键相机
输入整帧禁用。
...
成功不单独等价于画面验收通过。
标称值按已应用的鼠标标定换算输入增量；cooked UE 的灵敏度和相机 lag 仍会影响最终
角速度，所以验收以 `ue-final-pov` 实际变化为准，不把标称值冒充精密 deg/s。
...
安全互锁。F6 切换同样要求 WASD、Q/E 和左摇杆全部回中；切换帧先发送 IDLE，切换完成后
仍需重新按键或重新推动摇杆，持有旧输入不会穿过模式边界。
...
0.20/0.80/2.50 m/s、双击 boost 为 0.30/1.00/2.75 m/s；手柄在原生 `SLOW_WALK` 内连续映射，单独配置的上限默认是
