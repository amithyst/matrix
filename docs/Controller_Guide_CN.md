...
设置面板。面板区分“当前已应用”和“下次启动”配置：

| 按键 | 行为 |
|---|---|
| **M** | 在 `Local` 与 `Remote` 间切换下次启动配置 |
| **- / +** | 以 0.1 为步长调整 Remote 速度，范围 0.2x–1.0x |
| **F9** | 保存成功且存在待应用变更时，安全重启完整 Matrix/SONIC 运行链并应用 |
...
F9 会先要求 ESC/F9 已松开、设置页处于活动状态且已经发出安全中立帧，然后由最外层
launcher 重启**整条**运行链；不要单独重启 UE。重启期间保持所有控制输入松开。
