...
当 `MATRIX_SONIC=1` 且使用 `--control-source game` 时，launcher 现在默认进入 cooked
包内机器人自带的原生相机，不再把初始视角留在自由的 `Spectator_C`。UE 启动时会关闭
SpringArm 的相机延迟和旋转延迟、开启 SpringArm 碰撞，并针对真正渲染出来的机器人
Actor 执行 `viewclass`：
...
“以机器人为中心的视角选择”已经在河源的 live cooked runtime 上实测：可见画面中
机器人位于中央，PlayerController 的 ViewTarget 回读为 `MujocoSim_Custom_C`，live
custom SpringArm 的父组件回读为 `MainBody`，相机延迟、旋转延迟、碰撞检测分别回读为
`False`、`False`、`True`。这是 cooked UE 里的真实相机链，不是只停留在 Python 里的
相机契约。

这个结果**尚不能证明**移动中的平移跟随验收、每次鼠标拖动都是真正围绕机器人的
orbit，也不能证明仰俯角、墙面/地面防穿模恢复已经达到商业二游效果；V 也仍没有权威
模式回读。这些仍须让机器人移动并靠近障碍物完成黑盒验收，不能把当前实现称为完整的
“原神式相机 bridge”。

启动行为可以回滚，而且只在上述模式中生效：

- 设置 `MATRIX_GAME_CENTERED_CAMERA=0` 可完全禁用；
- 设置 `MATRIX_GAME_CAMERA_VIEW_CLASS=AnotherRobot_C` 可覆盖目标短 Blueprint 类名。
  值必须是以 `_C` 结尾的单个 token；空格、逗号和控制台分隔符都会被拒绝；
- `MATRIX_UE_EXTRA_EXEC_CMDS` 始终最后追加，因此操作者可以有意覆盖默认命令。

三条 `set Engine.SpringArmComponent ...` 是 UE 的 class-wide 控制台操作，并不只作用于
