...
它有完整的道路、建筑、路灯、绿化、车辆和行人视觉。锁定的 MuJoCo 来源有 869
个原生静态环境几何体；活动 SONIC 派生场景精确删除四条 Town10 外圈碰撞墙，
保留无限 `floor`，因此实际使用 865 个静态环境几何体。它比 ApartmentWorld
或自制厂房更接近真实都市，但还不是经过 2080 科幻美术改造的 Overworld。
...
派生变换 `town10-open-boundary-v1` 只删除 `ps_Cube`、`ps_Cube2`、
`ps_Cube3`、`ps_Cube4`。每条墙约为 250 m × 0.1 m × 3 m；变换在原始场景
SHA 和四条墙的尺寸、位置、旋转、碰撞掩码发生漂移时失败关闭。`floor` 必须继续
是启用碰撞的无限 plane。开放物理边界不代表 UE cooked 视觉、静态导航包或地图外
障碍已经扩展；走出城市视觉范围后可能出现视觉/物理不一致。

## G1 材质

通用导入流水线 V18 会识别标准 G1，并默认选择注册表中的 `unitree-stock`：
黑色软胶、石墨结构件、冷白外壳和银灰点缀。已验收的蓝 / 黑 / 白外观继续作为
`matrix-blue` 皮肤保留，可通过 `--skin matrix-blue` 切换。颜色、粗糙度、
金属度、部件匹配、增加黄金等新皮肤的方法，以及 Matrix UE 当前只渲染 RGBA 的
边界均记录在 `docs/G1_FINAL_MATERIALS_CN.md`。皮肤在每次生成 active XML 时
重写，因此切换不需要重新导入 mesh；材质转换不改变 collision 的几何/接触属性、
质量、惯量、关节或 SONIC 控制。
