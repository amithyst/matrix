...
SONIC 物理侧会在派生模型中应用 `moon-dynamic-ground-static-v2`：保留官方
`scene_terrain_moon_dynamic.xml` 的 256 个月面碰撞块，但移除这些地块自身的 freejoint，
让它们作为静态碰撞地形参与接触。这样 G1 的 SONIC body-only 状态维度保持可控，不把月面地块
的自由度混入机器人控制状态。官方 256 块地面只覆盖约 `1.6m × 1.6m`；V2 还会在
`z=-0.005m` 添加无限静态支撑平面，避免机器人跨出这块小网格后跌入无底区域。
...
物理模型中被静态化，manifest 会记录 `scene_transform=moon-dynamic-ground-static-v2`、
`scene_transform_contract.support_plane` 和 `staticized_freejoint_bodies`。V2 的 world
revision 与 V1 隔离，V1 中已经坠落或被拒绝的 checkpoint 不会被继续加载。
