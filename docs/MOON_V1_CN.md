...
`moon-v1` 是 SOL-2080 箱庭世界的第一版非地球场景。它复用 Matrix v0.1.2 官方
`MoonWorld` cooked PAK，不重做月面资产，也不反打包编辑上游内容。启动链路仍然走
`scripts/run_matrix_sonic.sh`，`scripts/run_matrix_sonic_moon_v1.sh` 只是固定 `--scene 15`
的薄包装。
...
```bash
cd /home/kaijie/worktrees/matrix-moon-v1
...
MoonWorld 视觉通过不等于月球物理通过。Matrix 官方
...
通过前，报告中必须写成 `Earth-gravity visual smoke` 或 `Moon visual smoke`，不能写成
完整月球动力学 benchmark。

ESC 的星体导航页仍是同一 SOL-2080 宇宙下的箱庭目的地入口。当前月球优先支持直接启动
和本箱庭内传送点；从城市箱庭一键跨场景切到 MoonWorld 需要后续扩展 destination 的
scene/map 路由，不在本 V1 中冒充完成。
