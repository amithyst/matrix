...
| [Cesium Native](https://github.com/CesiumGS/cesium-native) / [Cesium for Unreal](https://github.com/CesiumGS/cesium-unreal) | Apache-2.0 | 椭球、地理坐标、原点重定位和 UE 集成参考 | 与 Matrix 的局部物理/大世界渲染分层一致；审计 revision `f71495e9` |
...
历表状态覆盖。地球默认 `earth-wet-cloudy-v1`，并保留晴天/小雨 A/B。月球真空和火星
尘暴只定义兼容 profile，不能替代尚未部署的星体地图和 UE 大气实现。每个 profile 都以
