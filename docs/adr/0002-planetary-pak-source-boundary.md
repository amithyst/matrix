...
- 日期：2026-07-22
- 适用：Matrix 0.1.2、UE 5.5.4 Linux、SOL-2080 MoonWorld、箱庭世界
...
采用三层混合架构，不把“公开下载”“开源代码”“可编辑 UE 原资产”混为一谈。2026-07-22
起近期路线改为箱庭世界优先：MoonWorld 是第一批非地球箱庭；MarsWorld 暂停，直到找到
可运行官方/内部包或启动我们自有内容工程。
...
为新的星体箱庭和我们拥有来源的视觉资产建立一个小型、可编辑、可复现的 UE 内容工程。
...
MarsWorld 当前暂停。恢复时必须采用预烘焙区域，不在 runtime 依赖 Cesium C++：
...
### Phase B：自有星体箱庭 content-only PAK

Mars 暂停期间不启动本阶段。恢复条件是找到可审计的火星原始资产和 UE 5.5.4 cook 环境，
并明确它服务于箱庭地图，不冒充整颗火星或无缝太阳系。
...
### Phase C：无缝宇宙 renderer

只有箱庭世界无法满足连续航行、整星 streaming 和动态 renderer readback 时启动。先取得
