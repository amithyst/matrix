...
因此“Matrix 有月球地图”和“当前机器 ESC 可以传送到月球”仍不是同一件事。箱庭世界第一阶段
复用官方 MoonWorld，不重做月面；启动时会核验 chunk 26 release、动态高度图和物理 XML，
缺任一项就保持按钮不可用。还需在每台目标机分别完成 SONIC 物理、低重力、相机和持久化
验收。具体命令见
