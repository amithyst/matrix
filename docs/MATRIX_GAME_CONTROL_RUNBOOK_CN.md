...
- 键盘 WASD 使用按住修饰键的静走/普通走/跑步三档：依次为 Ctrl、无修饰键、Shift；
  三档都保持原生 SONIC `SLOW_WALK`；
- Q/E 不再参与机器人 yaw；
- 精确 UE PID 失焦、观察到的 V 自由相机切换、鼠标拖相机、输入过期、断连和
  provider 故障都会停机；
...
- 回读 UE 真实自由相机/输入模式；当前 V 只做 best-effort 镜像。
...
| 最高速度 | 0.30 m/s |
| 键盘速度档 | Ctrl 静走 0.10；无修饰键普通走 0.20；Shift 跑步 0.30 m/s；冲突时 Ctrl 优先 |
| 原生步态 | `SLOW_WALK`；0.10 m/s 步态下限，并带启停滞回 |
...
河源首轮标定不要提高超时和速度；保留启动弹性带，并保留 launcher 默认的跌倒即失败、
零数值重置等门禁。内部速度意图受限幅，但原生步态启停存在 0.10 m/s 下限跳变。
内部意图会先爬升到入口，首个原生 floor 帧之后发布速度再次遵守配置限幅；需要记录
实际启停距离。
...
2. Ctrl+W、W、Shift+W 分别稳定在 0.10、0.20、0.30 m/s；Ctrl+Shift 冲突时使用
   更安全的 0.10 m/s，移动中切换档位仍遵守加减速度限幅；
3. A、D、S 都会让机器人朝运动方向转身；180 度反向时先转身，再逐渐建立平移；
4. 单独按 Q 或 E 不改变 root，也不改变 game-control heading；
5. 正常松键按减速度平滑停止，安全事件则立即硬停。
...
顺时针、逆时针连续绕转几圈后重复测试；把指针移到各个屏幕边缘后重复；V 自由相机
进入、退出后也重复。只要出现累计偏差、光标 warp 后突跳、或 UE 自动回正造成不一致，
...
进入安全设置页，点击 Remote、用大号 -/+ 选择 0.2x–1.0x，再点击“返回游戏并应用”
（或按 Enter）；页面会等待安全中立帧后自动重载完整运行链，F9 仅作为键盘兜底。
重启后确认面板的 `CURRENT APPLIED (SDL)` 与目标一致，并重新执行四方向与
多轮往返测试。F10/F12 仍属于外部 MouseLock，不是 Matrix 设置页的绑定。

Remote 0.4x 是 SDL/UE 原生倍率；示例基值 0.12 度/像素乘后，状态会显示
`x11-mirror = 0.048 度/像素`，但这只是 X11 根窗口像素上的名义算术值，不是可见相机
的实测灵敏度。
...
| provider 启动后按 V | 观察到按键沿时立即归零并显示 `free_camera`；UE 模式回读完成前只能算 best-effort |
...
完成回中复位，以及一次失焦/恢复。V 必须做完整循环：进入自由相机、确认硬停、再次
按 V 退出、再完成 neutral re-arm；只切换一次会正确地停在 safe stop，无法通过边界。
最后保证有效位移达到 lock 要求。
